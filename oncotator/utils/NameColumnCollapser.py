...
class NameColumnCollapser(ColumnCollapser):
    """This class takes multiple values in an annotation (separated by "|") and collapses into a single value based on given rules.

    Takes named columns from an input config file and collapses based on the specified rule in the config file.
    ...
    def __init__(self, config_file="column_collapse.config"):
        ...
        for c in self._columns_to_collapse:
            ...
            if self._method_dict[c] is None or self._method_dict[c] == "":
                self._method_dict[c] = NameColumnCollapser.MEAN

        # Basic validation
        problematic_method_assignments = dict()
        for c in self._columns_to_collapse:
            if self._method_dict[c] not in NameColumnCollapser.VALID_VALUES:
    ...
    def _collapse_column(self, column_name, current_value):
        ...
        if self._method_dict[column_name] == NameColumnCollapser.MIN:
            return str(min(final_vals))

        elif self._method_dict[column_name] == NameColumnCollapser.MEAN:
