...
def parse_gaf(gaf_file):
    ...
    for g in gaf_file:
        ...
        if g.featureType == 'transcript' and g.compositeType == 'genome':
            ...
            record['transcript_coords'] = [(int(c.split('-')[0]),int(c.split('-')[1])) for c in g.featureCoordinates.split(',')]
            ...
            record['genomic_coords'] = [(int(c.split('-')[0]),int(c.split('-')[1]),) for c in genomic_coords.split(',')]
            ...
            if g.featureInfo != '':
                ...
                if len(feature_info_lst) == 1:
                    ...
                    record['confidence'], record['cds_start'], record['cds_stop'] = [int(c.split('=')[1]) for c in g.featureInfo.split(';')]
		...
		for bin in binned_genes[chr]:
			for genedict in binned_genes[chr][bin]:
				...
				for i,locus in enumerate(genedict['alt_loci']):
					if locus['chr'] == genedict['chr'] and locus['start'] == genedict['start'] and locus['end'] == genedict['end']:
						to_delete.append(i)
