...
def add_full_mut_key_columns(input_df, chr_col_name='Chromosome', start_col_name='Start_position',
		end_col_name='End_position', ref_col_name='Reference_Allele', alt_col_name='Tumor_Seq_Allele1'):
	...
	input_df['full_mut_key'] = chrom_series + '_' + start_series + '_' + end_series + '_' + ref_series + '_' + tum_series
...
def main(tcga_mutations_fname, output_fname):
	...
	get_alt_allele = lambda x: x['tumor_seq_allele_1'] if x['tumor_seq_allele_1'] != x['reference_allele'] else x['tumor_seq_allele_2']
	tcga_df['alt_allele'] = tcga_df.apply(get_alt_allele, axis=1)
	
	add_full_mut_key_columns(tcga_df, chr_col_name='chromosome', start_col_name='start_position', end_col_name='end_position',
	...
	for tcga_disease in all_tcga_diseases:
		total_disease_sample_counts[tcga_disease] = len(set(tcga_df[tcga_df.tcga_disease == tcga_disease].tumor_sample_barcode))
	...
	output_headers = ['chr', 'start', 'end', 'ref_allele', 'alt_alele', 'tcga_mut_count', 'tcga_muts_disease_names', 'tcga_muts_disease_counts', 'tcga_muts_disease_percents']
	...
	for mut_key in all_mut_keys:
		...
		output_fields.extend([str(tcga_mut_count[mut_key]), tcga_muts_disease_names[mut_key], tcga_muts_disease_counts[mut_key],
