...
def main(cosmic_mutations_fname, output_fname):
	...
	for mut_key in indexed_cosmic_data:
		...
		disease_data = sorted([(k, v[0], v[1]) for k, v in indexed_cosmic_data[mut_key].items()], key=lambda x: x[1], reverse=True)
	...
	output_headers = ['chr', 'start', 'end', 'ref_allele', 'alt_alele', 'cosmic_mut_count', 'cosmic_muts_disease_names', 'cosmic_muts_disease_counts', 'cosmic_muts_disease_percents']
	...
	for mut_key in all_mut_keys:
		...
		output_fields.extend([str(cosmic_mut_count[mut_key]), cosmic_muts_disease_names[mut_key], cosmic_muts_disease_counts[mut_key],
