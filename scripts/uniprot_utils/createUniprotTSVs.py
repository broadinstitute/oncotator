    ...
    for db in gene_ids_2_entrynames:
        for s in eval(db + '_data').values():
            for c in s.cross_references:
                if c[0] == 'GeneID':
                    gene_ids_2_entrynames[db][c[1]].append(s.entry_name)
    
    
    for b in genes.values():
        for k in b.keys():
            for g in b[k]:
                if g['id'] in gene_ids_2_entrynames['swiss']:
                    ##add swiss-prot data to dict
                    entry_names = list(gene_ids_2_entrynames['swiss'][g['id']])
                    g['uniprot_entry_name'] = entry_names.pop(0)
                    if entry_names:
                        g['alt_uniprot_entry_names'] = entry_names
                    g['uniprot_accession'] = swiss_data[g['uniprot_entry_name']].accessions[0]
                    if len(swiss_data[g['uniprot_entry_name']].accessions) > 1:
                        g['alt_uniprot_accessions'] = swiss_data[g['uniprot_entry_name']].accessions[1:]
                    
                    g['uniprot_db_source'] = 'swiss'
                    continue
                    
                elif g['id'] in gene_ids_2_entrynames['trembl']:
                    ##add trembl data to dict if none found in swiss-prot first
                    entry_names = list(gene_ids_2_entrynames['trembl'][g['id']])
                    g['uniprot_entry_name'] = entry_names.pop(0)
                    if entry_names:
                        g['alt_uniprot_entry_names'] = entry_names
                    g['uniprot_accession'] = trembl_data[g['uniprot_entry_name']].accessions[0]
                    if len(trembl_data[g['uniprot_entry_name']].accessions) > 1:
                        g['alt_uniprot_accessions'] = trembl_data[g['uniprot_entry_name']].accessions[1:]
                    g['uniprot_db_source'] = 'trembl'
    return genes

def correct_uniprot_ids_hack(genes, swiss_data):
    #replacement_dict = {
    ...
    for k1,v in genes.items():
        for k in v.keys():
            for g in v[k]:
                if v.get('uniprot_db_source','') == 'trembl': 
                    genes_with_trembl_id.append(k)
        ...
        for k in b.keys():
            for g in b[k]:
                if g['gene'] in replacement_dict:
                    ...
                    g['uniprot_entry_name'] = replacement_dict[g['gene']][0]
                    g['uniprot_accession'] = replacement_dict[g['gene']][1]
                    g['uniprot_db_source'] = 'swiss'
    return genes
...
def renderSimpleUniprotTSV(genesDict, outputFilename):
...
if __name__ == '__main__':
    ...
    genesDict = create_gaf_dicts(gaf_fname)[1]
